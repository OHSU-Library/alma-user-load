CREATE DEFINER=`libadm`@`10.%` PROCEDURE `SIS_XML_Create_Outfile_FULL`(IN filename VARCHAR(256))
BEGIN

IF filename = '' THEN SET filename = CONCAT('/home/users/exlibdd/MySQL/SISFULL_', DATE_FORMAT(NOW(), '%Y%m%d-%H%i%s'), '.xml'); END IF;
...
SET @sql =
	CONCAT(
        ...
        WHERE primary_id LIKE '%@ohsu.edu'
